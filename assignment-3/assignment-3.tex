...
%
\begin{lstlisting}
https://github.com/asishcs2011010/demo/blob/main/assignment-3/assignment-3.tex 
\end{lstlisting}
\section*{question no}
...
let $X \in\ \{0,1\}$ be the random variable  which denotes\\ whether the picked up ball is red  or green .\\
 1 represents picking up of red ball\\
 0 represent picking up of  green ball\\
\begin{enumerate}
    
\item for the first draw
\begin{align}
  \begin{tabular}{| c| c| c| }
  \hline
 X & $X=0$ & $X=1$\\ 
...
\end{tabular}
\end{align}\\
the probability that the first ball drawn is red is Pr$(X=1) = 1/2$
\item For the second draw 
\begin{align}
    \begin{tabular}{| c| c| c| }
  \hline
 X & $X=0$ & $X=1$\\ 
...
\end{tabular}
\end{align}\\
the probability that the second ball drawn is green is Pr$(X=0) = 8/15$\\

The probability that the first ball drawn is red and the second ball drawn is green is  = $(1/2)\times(8/15) = 4/15 $\\\\
\end{enumerate}
\end{document}
© 2021 GitHub, Inc.
Terms
